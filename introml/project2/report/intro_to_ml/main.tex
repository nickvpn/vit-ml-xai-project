...
\begin{abstract}
We study random-feature ridge regression on a tabular regression task and use the same pipeline to connect three foundational ideas. First, we show that random Fourier features (Rahimi and Recht, 2007) approximate exact RBF kernel ridge regression as the number of features $P$ grows. Second, we sweep $P$ across the interpolation threshold $P = N$ and reproduce the classical double-descent test-error curve (Belkin, Hsu, Ma, and Mandal, 2019) on California Housing. Third, we test the three-factor explanation of double descent from Schaeffer et al.\ (2023) by ablating each factor in turn: ridge regularization, leading-singular-direction projection, and noiseless targets. Each ablation eliminates the spike at $P = N$, confirming that all three factors contribute. We pair this with kernel-PCA spectral analysis showing the smallest singular value of the random feature matrix collapsing to near zero at $P = N$, which is the structural source of the spike.
\end{abstract}

\section{Introduction}

The bias-variance tradeoff teaches that increasing model capacity beyond the interpolation threshold should overfit. The double-descent phenomenon (Belkin et al., 2019) shows that for many overparameterized models, test error decreases again as capacity grows past the threshold, producing a characteristic two-descent curve with a spike at $P \approx N$. Schaeffer et al.\ (2023) argue that the spike is driven by three factors that act in concert: (a) the smallest singular value of the feature matrix vanishing as $P \to N$, amplified when (b) test-feature variation is concentrated in the trailing singular directions, and only triggered at all when (c) the target contains residuals that are not in the column space of the training features. Removing any one factor flattens the spike.

This project reproduces the double-descent curve with random Fourier features on a tabular regression task and tests the three-factor explanation by ablating each factor in turn. The same random-feature pipeline approximates the exact RBF kernel ridge regressor as $P \to \infty$, providing the kernel-approximation reference point that motivated random features in the first place.
...
Rahimi and Recht (2007) introduced random Fourier features as a stochastic approximation to translation-invariant kernels: sampling random frequencies $\omega_i \sim \mathcal{N}(0, \sigma^{-2} I)$ and phases $b_i \sim \mathrm{Unif}(0, 2\pi)$, the map $\phi(x)_i = \sqrt{2/P}\, \cos(\omega_i^\top x + b_i)$ produces a feature embedding whose inner product approximates the RBF kernel as $P$ grows.
...
\paragraph{Dataset.}
California Housing (Pace and Barry, 1997, via \texttt{sklearn.datasets.fetch\_california\_housing}) provides $20\,640$ rows with 8 numeric features and a continuous target. We hold out $20\%$ of the rows as a fixed test set ($N_{\text{test}} = 4128$) and take a small training subset of $N = 256$ rows from the remaining $80\%$. Both features and target are standardized using statistics from the training subset only. Holding $N$ small makes the interpolation threshold $P = N$ cheap to span.
...
We sweep $P \in \{4, 8, 16, 32, 64, 128, 200, 240, 256, 272, 300, 350, 512, 1024, 2048, 4096\}$, with three random-feature seeds per $P$. For each $P$, we fit ridge regression with $\lambda = 10^{-3}$ as the default. Closed-form solution: in the underparameterized regime $P < N$ we solve $(\Phi^\top \Phi + \lambda I) w = \Phi^\top y$. In the overparameterized regime $P \geq N$ we use the dual form $w = \Phi^\top (\Phi \Phi^\top + \lambda I)^{-1} y$ for numerical stability.
...
\paragraph{Class-method benchmarks.}
...
\begin{itemize}
  ...
  \item \textbf{Factor (c), noiseless target.} Replace $y$ with $y_{\text{clean}} = w_{\text{lin}}^\top x$, the best linear fit on raw features. The new target sits exactly in the column space of any random feature map, so there are no residuals to amplify.
\end{itemize}
