...
Random Fourier features are also useful in their own right as a scalable kernel approximation. Exact kernel methods compute and store an $N \times N$ kernel matrix at $O(N^2)$ memory and $O(N^3)$ training cost, which becomes prohibitive once $N$ grows past a few thousand. Rahimi and Recht (2007) showed that sampling $P$ random features produces an explicit $P$-dimensional embedding whose inner product converges in expectation to the kernel value as $P$ grows, so random-feature ridge converges to exact kernel ridge while costing only $O(NP)$. We use this convergence as a sanity check on the pipeline, anchoring the random-feature ridge sweep against an exact RBF kernel ridge baseline at the same bandwidth.
