...
\paragraph{Data.}
SALICON provides $10\,000$ training and $5\,000$ validation COCO images with continuous fixation density maps. We use the validation split for all representation analysis. Activation collection (probes, CKA, clustering) uses a fixed 1000-image subset, SAE ablation uses 200 images, and the Grad-CAM and SHAP attribution baselines use 100 images. All images are resized to $224 \times 224$ and ImageNet-normalized. Saliency maps are resized to $14 \times 14$ for low-resolution probing and to $224 \times 224$ for SHAP alignment.
